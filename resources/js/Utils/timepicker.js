/*
* Code borrowed from: https://github.com/ZulNs/Timepicker.js
* Custom modifications:
* - deleted the "Pick" button
* - changed "light" theme
* - changed styles for picker in "create" method
* - use custom font
* - changed timezone offset to GMT+2
* - onPicked fire event after touch/drag to set time
* - changed onPick method
* - added "id" to timeStr in "create" method
*/
export default function Timepicker(isClk,is24H,isLight,hour,minute){
	isClk=!!isClk;
	is24H=!!is24H;
	isLight=!!isLight;
	...
	onMouseUp=function(e){
		if(isDragging&&isFiredByMouse){
			...
			e.preventDefault();
			isDragging=false
			// fire onPicked event to set time
			onPick()
		}
	},
	...
	create=function(){
		...
		tpick.style.cssText='position:relative;width:240px;margin:0 auto;';
		...
		//timeStr.style.cssText='position:absolute;left:60px;top:248px;width:120px;text-align:center;font:14px Verdana;cursor:default';
		timeStr.style.cssText='position:absolute;left:35px;top:248px;width:170px;text-align:center;font:16px CustomClockFont;color:#e36386;cursor:default;visibility: hidden';
		timeStr.id = 'timedisplay';
		...
		tpick.style.height=isClk?'240px':'305px';
		if(isClk){
			timeStr.style.display = 'none';
			updClkTm()
		}
		else{
			addEvts();
	...
	setClkTheme=function(){
		...
		ctx.strokeStyle=isLight?'#e36386':'#fff';
		ctx.beginPath();
		...
		ctx.fillStyle=isLight?'#e36386':'#fff';
		for(let i=0;i<12;i++){
			ctx.beginPath();
		...
		ctx.fillStyle=isLight?'#e36386':'#2196F3';
		ctx.beginPath();
		ctx.moveTo(10,0);
		ctx.lineTo(0,90);
		ctx.lineTo(20,90);
		...
		ctx.fillStyle=isLight?'#e36386':'#ffeb3b';
		ctx.beginPath();
		ctx.moveTo(6,0);
		ctx.lineTo(0,110);
		ctx.lineTo(12,110);
	...
	this.setClockMode=function(m){
		if(typeof m=='boolean'&&m!=isClk){
			...
			timeStr.style.display=isClk?'none':'';
			tpick.style.height=isClk?'240px':'320px';
			if(isClk){
				remEvts();
				hour=Timepicker.getHours();
...
Timepicker.tzOffset=Date.parse('01 Jan 1970 GMT+2');
